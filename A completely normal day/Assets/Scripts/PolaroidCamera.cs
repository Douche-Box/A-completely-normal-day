...
public class PolaroidCamera : MonoBehaviour
{
    ...
    public event Action<Polaroid> OnCurrentPhotoChanged;

    [SerializeField] Polaroid _currentPolaroid;
    public Polaroid CurrentPhoto
    { get { return _currentPolaroid; } set { _currentPolaroid = value; OnCurrentPhotoChanged?.Invoke(value); } }

    [SerializeField] List<GameObject> _objectsInView = new List<GameObject>();

    [SerializeField] Collider _frontCollider;
    public Collider FrontCollider
    { get { return _frontCollider; } }
    ...
    public void TakePhoto()
    {
        if (_canTakePhoto)
        {
            ...
            Transform newPhoto = Instantiate(_photoPrefab, _photoSpawnPoint.position, _photoPrefab.transform.rotation).transform;
            ...
            Polaroid polaroid = newPhoto.GetComponentInChildren<Polaroid>();
            polaroid.CurrentPolaroidCamera = this;

            UpdateObjectsInView();

            CheckForCluePoints(polaroid);
